...
import { SendHorizonal, Bot, ArrowLeft, Plus, MessageSquare, LogOut } from "lucide-react";
...
export function AnalyticsChatView({ onBack }: AnalyticsChatViewProps) {
  ...
  const generateResponse = (userQuery: string) => {
    if (!data) return "Unable to access analytics data at the moment. Please try again later.";
    
    const traineesCount = data.traineeCountPerTraining 
      ? data.traineeCountPerTraining.reduce((sum, training) => sum + training.totalTraineeCount, 0)
      : 'unknown';
    ...
    <div className="flex h-screen w-full">
      ...
      <div className="w-64 h-full bg-white border-r overflow-auto flex flex-col">
        ...
        <div className="p-4 border-b flex items-center">
          <Button variant="ghost" size="sm" className="text-brand" onClick={onBack}>
